<script lang="ts">
  ...
  import { reconstructFile, getFileTypeFromMime } from "../lib/utils/walrus.js";

  import FileDropZone from "./uploader/FileDropZone.svelte";
  import UploadActions from "./uploader/UploadActions.svelte";
  ...
  async function uploadToWalrus() {
    ...
    try {
      ...
      for (let i = 0; i < totalChunks; i++) {
        ...
        if (!response.ok)
          throw new Error(
            `HTTP error! status: ${response.status} on chunk ${i + 1}`
          );
      ...
      if (user && user.address) {
        try {
          // Check if upload already exists (optional, but good practice from frontend)
          const existingUpload = await apiServices.getUploadByBlobId(blobId);

          if (existingUpload) {
            console.log(
              "Upload already exists, reusing metadata:",
              existingUpload
            );
...
<div
  class="mx-auto w-full max-w-4xl overflow-hidden rounded-3xl border border-odoben-primary/10 bg-odoben-surface/50 shadow-2xl shadow-odoben-bg/20 backdrop-blur-xl mb-40"
>
  <!-- Header -->
  <div
  ...
  >
    <h1 class="mb-2 text-4xl font-bold">🐋 Upload File</h1>
    ...
    <div in:fade>
      {#if !selectedFile}
        ...
        <FileDropZone
          ...
          on:fileSelected={(e) => handleFileSelect(e.detail)}
        />
      {:else}
        ...
        {#if !uploadResult}
          <div class="mb-8 space-y-6">
            <!-- Privacy Settings -->
            <div
              class="rounded-xl border border-odoben-primary/10 bg-odoben-bg/30 p-6"
            >
              <h3
                class="mb-4 flex items-center gap-2 text-lg font-semibold text-odoben-text"
              >
              ...
              <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div class="space-y-2">
                  <label
                    ...
                    class="text-sm text-odoben-text/60">Visibility</label
                  >
                  <select
                ...
                {#if visibility === "paid"}
                  <div class="space-y-2" in:fade>
                    <label for="price-input" class="text-sm text-odoben-text/60"
                      >Price (SUI)</label
                    >
                    <div class="relative">
                      <input
                      ...
                      />
                      <span
                        class="absolute top-1/2 left-4 -translate-y-1/2 text-odoben-text/60"
                        >💧</span
                      >
                    </div>
                  </div>
            ...
            >
              ...
              <div class="flex justify-center">
                ...
                {:else}
                  <div
                    class="flex items-center gap-4 p-4 rounded-xl bg-odoben-bg/50 border border-odoben-primary/10 w-full max-w-md"
                  >
                    <div
                      class="w-12 h-12 bg-odoben-primary/20 rounded-lg flex items-center justify-center text-2xl"
                    >
                    ...
                    <div class="flex-1 min-w-0">
                      <p class="font-medium text-odoben-text truncate">
                        {selectedFile?.name}
                      </p>
        ...
        {#if uploadResult}
          <div
            class="rounded-xl border border-odoben-primary/10 bg-odoben-bg/30 p-6 mt-8"
          >
            <h3
              class="mb-4 flex items-center gap-2 text-lg font-semibold text-odoben-text"
            >
            ...
            {#await reconstructFile(uploadResult.blobId)}
              <div
                class="mb-4 p-4 rounded-lg bg-odoben-bg/50 border border-odoben-primary/10 flex items-center justify-center gap-2 text-odoben-text/60"
              >
                <div
                  class="w-4 h-4 border-2 border-odoben-primary/30 border-t-odoben-primary rounded-full animate-spin"
                ></div>
                Verifying upload...
              </div>
            {:then result}
              ...
              >
                ...
                {:else if getFileTypeFromMime(result.type) === "video"}
                  <MediaPlayer src={result.url} type="video" />
            ...
            {:catch error}
              <div class="mb-4 text-sm text-odoben-accent">
            ...
            <div class="space-y-2">
              <p class="text-odoben-text/80">
                ...
                <code class="rounded bg-odoben-bg/50 px-2 py-1 text-sm"
                  >{uploadResult.blobId}</code
                >
              </p>
              <p class="text-odoben-text/80">
                ...
                <a
                  ...
                  target="_blank"
                  class="break-all text-odoben-primary hover:underline"
                  >{uploadResult.url}</a
                >
              </p>
              {#if uploadResult.slug}
                <p class="text-odoben-text/80">
                  ...
                  <a href="#" class="text-odoben-primary hover:underline"
