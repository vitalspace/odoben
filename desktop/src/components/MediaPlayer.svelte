...
>
  ...
  <!-- Controls Overlay -->
  <div
    class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4 transition-opacity duration-300 {showControls
      ? 'opacity-100'
      : 'opacity-0'}"
  >
    <!-- Progress Bar -->
    <div
      class="mb-4 group/progress relative h-1 cursor-pointer bg-white/20 rounded-full"
    >
      <div
  ...
  {#if !isPlaying && type === "video"}
    ...
    >
      <div
        class="bg-black/50 rounded-full p-4 backdrop-blur-sm border border-white/10"
      >
